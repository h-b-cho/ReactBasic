...
function App() {
  ...
  const [booltodo, setBooltodo] = useState(false);

  useEffect(() => {
    fetch("http://localhost:80/api/todo")
  ...
  const onSubmitHandler = (e) => {
    e.preventDefault();
    const tt = e.target.text.value;
    const dd = e.target.done.checked;
    fetch("http://localhost:80/api/todo", {
      method: "POST",
      headers: {
      ...
      <ThemeContext.Provider value={{ isDark, setIsDark }}>
        ...
        <form onSubmit={onSubmitHandler}>
          ...
          <input
            ...
            type='checkbox'
            value={booltodo}
            onChange={(e) => setBooltodo(e.target.value)}
          ></input>
        ...
        {todolist.map((item) => (
          <ul key={item.id}>
            <li>{item.id}</li>
            <li>{item.text}</li>
            <li style={{ color: isDark ? "black" : "red" }}>{item.done ? "Y" : "N"}</li>
          </ul>
        ))}
